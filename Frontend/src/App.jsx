...
function App() {
  ...
  const printReceipt = (payment, member) => {
    ...
    receiptWindow.document.write(
      `<html><head><title>Receipt</title><style>body{font-family:monospace;padding:20px;text-align:center} .divider{border-bottom:1px dashed #333;margin:15px 0} .row{display:flex;justify-content:space-between;text-align:left}</style></head><body><h2>GYM FITNESS</h2><p>Official Receipt</p><div class="divider"></div><div class="row"><span>Receipt #:</span><strong>${transactionId}</strong></div><div class="row"><span>Date:</span><strong>${formatSafeDate(payment.payment_date, true)}</strong></div><div class="row"><span>Member:</span><strong>${member.full_name}</strong></div><div class="divider"></div><div class="row" style="font-size:18px"><span>TOTAL:</span><strong>₱${parseFloat(payment.amount).toFixed(2)}</strong></div><div class="divider"></div><p>Thank you for grinding!</p></body></html>`,
    );

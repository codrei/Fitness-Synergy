...
function PromosManager({ theme }) {
  ...
  const handleSave = async (e) => {
    ...
    const res = await apiFetch(endpoint, { method: "POST", body: JSON.stringify(form) }).then((r) => r.json());
  ...
  const handleDelete = async (promo) => {
    if (!window.confirm(`Delete promo "${promo.promo_name}"? This cannot be undone.`)) return;
  ...
  return (
    <div style={{ padding: "30px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
        ...
        <button
          onClick={openAdd}
          style={{ padding: "10px 20px", backgroundColor: theme.primary, color: theme.primaryText, border: "none", borderRadius: "8px", cursor: "pointer", fontWeight: "bold", fontSize: "14px" }}
        >
          ➕ Add New Promo
        </button>
      </div>
      ...
      <div style={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: "12px", overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {promos.length === 0 && (
              <tr><td colSpan={5} style={{ padding: "40px", textAlign: "center", color: theme.textMuted }}>No promos found.</td></tr>
            )}
            {promos.map((promo, i) => (
              <tr key={promo.promo_id} style={{ borderTop: `1px solid ${theme.border}`, background: i % 2 !== 0 ? `${theme.border}22` : "transparent" }}>
                ...
                <td style={{ padding: "14px 20px" }}>
                  <span style={{
                    ...
                    backgroundColor: promo.is_active == 1 ? "#22c55e22" : "#99999922",
                ...
                <td style={{ padding: "14px 20px", textAlign: "right", whiteSpace: "nowrap" }}>
                  <button
                    ...
                    style={{
                      padding: "7px 14px",
                      backgroundColor: promo.is_active == 1 ? "#6b7280" : "#22c55e",
                  ...
                  <button
                    ...
                    style={{ padding: "7px 14px", backgroundColor: theme.danger, color: "#fff", border: "none", borderRadius: "6px", cursor: "pointer", fontWeight: "bold", fontSize: "12px", opacity: deleting === promo.promo_id ? 0.6 : 1 }}
                  >
                    🗑️
                  </button>
                </td>
      ...
      {modal && (
        ...
        >
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
              <h2 style={{ margin: 0, fontSize: "18px" }}>{modal.mode === "add" ? "➕ Add New Promo" : "✏️ Edit Promo"}</h2>
            ...
            <form onSubmit={handleSave} style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              <div>
                ...
                <input
                  type="text" required value={modal.form.promo_name}
                  onChange={(e) => setModal((m) => ({ ...m, form: { ...m.form, promo_name: e.target.value } }))}
              ...
              <div>
                ...
                <input
                  type="text" value={modal.form.description}
                  onChange={(e) => setModal((m) => ({ ...m, form: { ...m.form, description: e.target.value } }))}
                  placeholder="e.g., Celebrate New Year with 15 free days!" style={inputStyle}
                />
              </div>

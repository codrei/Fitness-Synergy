...
function InstallmentPaymentModal({
  ...
  const paymentsInCycle = paymentHistory.filter(
    (p) =>
      !member.start_date ||
      new Date(p.payment_date) >= new Date(member.start_date),
  );
  const totalPaid = paymentsInCycle.reduce(
      ...
      >
        ...
        >
          ...
          >
            ...
            <div
              style={{
                fontWeight: "bold",
                color: outstanding > 0 ? theme.danger : "#22c55e",
              }}
            >
              ₱{fmt(outstanding)}
            </div>
        ...
        >
          ...
          >
            ...
            <span
              style={{ color: theme.primary, fontWeight: "bold", fontSize: 18 }}
            >
              ₱
              {thisPayment.toLocaleString("en-PH", {
          ...
          {error && (
            <div style={{ color: theme.danger, fontSize: "13px" }}>{error}</div>
          )}
